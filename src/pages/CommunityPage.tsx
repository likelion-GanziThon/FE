...
import PostCard from '@/components/common/PostCard'; // 사용자가 지정한 경로 유지
import type { Filters } from '@/types';
import { useState } from 'react';
import { MessageSquare, SlidersHorizontal, Search, X } from 'lucide-react';
...
export default function CommunityPage() {
  ...
  const { data: posts, isLoading } = useGetPosts('FREE', filters);
    ...
    <div className='animate-in fade-in min-h-screen bg-gray-50/50 pb-20 duration-500'>
      ...
      <div className='border-b border-gray-100 px-6 py-3'>
        <div className='mb-2 flex items-center gap-3'>
          <div className='bg-home-orange-100 rounded-lg p-2'>
            <MessageSquare className='text-home-orange-600 size-6' />
          </div>
          <h1 className='text-2xl font-bold text-gray-900'>자유 게시판</h1>
        </div>
        <p className='pl-1 text-sm text-gray-500'>이웃들과 자유롭게 소통하고 정보를 나누세요.</p>
      </div>
          ...
          <form
            ...
            <div className='flex items-start gap-2'>
              <div className='flex-1'>
                <SearchTextField /> {/* placeholder 등이 내부 구현되어 있다고 가정 */}
              </div>
      ...
      <div className='container mx-auto mt-6 flex max-w-2xl flex-col gap-4 px-4'>
        {isLoading ? (
          ...
          <div className='flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-white py-20 text-center'>
            ...
            <p className='font-medium text-gray-900'>등록된 게시글이 없어요</p>
            <p className='mt-1 text-sm text-gray-500'>
              다른 검색어를 입력해보거나, 첫 글을 작성해보세요!
            </p>
          </div>
        )}
      </div>
